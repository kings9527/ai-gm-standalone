...
> **截图**: 见 `docs/screenshots/architecture-diagram-high-res.png` — 高分辨率架构图，用于文档展示（建议使用 Draw.io / Excalidraw 导出）
...
> **截图**: 见 `docs/screenshots/dev-mode-three-terminals.png` — 开发模式下三个终端窗口（Vite + Express + Electron）
...
Response: text/plain (raw stream, not SSE — chunks are decoded directly via ReadableStream)
```
...
Body: { id?, name, author?, version?, system?, description?, content, style? }
...
POST   /api/settings             # 设置键值对（value 会被强制转为 String 存储）
```
...
// 注意：llmStream 在 electron.ts 中封装为 (body, onChunk, onEnd) 三参数
...
```typescript
await window.electronAPI.imageSearch(query);     // 搜索图片（仅传 query，type 默认 bg）
...
> **截图**: 见 `docs/screenshots/combat-skill-editor-code.png` — VS Code 中编辑战斗技能定义的截图，带语法高亮
